 ...
 */
function Parser(content, file, settings) {
    this.file = file;
    // 配置文件读取
...
Parser.prototype = {
    ...
    renderTpl: function() {
        ...
        // 获取#parse引入的文件
        opt.template = this.parser_template_block(opt.template);
    ...
    // 解析block分派对应的解析方法
    parser_template_block: function(content){
        var self = this,
            opt = this.opt,
            ld = this.opt.left_delimiter,
            rd = this.opt.right_delimiter,
            regParse = new RegExp('\\'+ld+'([^{'+rd+',=,%,\\s}]+)\\s+.*\\'+rd,'g');
            content = content.replace(regParse, function(match, action){
                var result = '';
                if(action === 'widget'){
                    result =  self.compileParse(match);
                }else{
                    result = match;
                }
                return result;
    ...
    compileParse: function(content) {
        var self = this,
            ...
            ld = this.opt.left_delimiter,
            rd = this.opt.right_delimiter,
            regParse = new RegExp('\\'+ld+'.*name\\s*=\\s*(\'|\")([^{=,\',\"}]+)(\'|\").*'+rd,'g');
        content = content.replace(regParse, function(match, qoute, uri) {
...
module.exports = function(content, file, opt) {
    return new Parser(content, file, opt);
};